...
def relative(df):
    if df.shape[0] > 0:
        ...
        df["words_deleted_relative_v_f"] = df.apply(
            lambda row: _relative(row["total_words_f"], row["words_deleted_v_f"]),
            axis=1,
        )
        df["words_deleted_relative_s_f"] = df.apply(
            lambda row: _relative(row["total_words_f"], row["words_deleted_s_f"]),
            axis=1,
        )
    return df
...
def calculate_median(rows):
    keys = [
        "jaccard_s_f",
        "jaccard_v_f",
        "words_added_relative_s_f",
        "words_added_relative_v_f",
        "words_deleted_relative_s_f",
        "words_deleted_relative_v_f",
    ]
    median_values = {}
